...
~~- 2026年5月1日前：发布第二批样例集；~~
- 2026年5月2日：发布[第二批样例集](sample%20sets/sample_20260502.json)（已完成）；
- 2026年6月1日-6月7日：发布正式评测集（在发布7天内开放下载）；各参赛队伍开展评测；
...
- 组织方正在计划开设一个leaderboard网站，网站上线后将承接报名和结果提交等功能。（预计五月三十一日上线）
- 在评测网站进行注册时，每一支参赛队伍只允许注册一个网站账号用于提交和评测。一旦发现参赛队伍存在违规注册行为，将取消该参赛队伍的参赛资格。
...
本次评测以JSON格式提供样例集和评测集。[样例集](sample%20sets)数据量为566条，评测集数量约在2000-4000条。语料由组织方团队筛选自相关真实语料库，并进行改编、标注与校对。
...
（4）	`hypothesis`：结论句，即被蕴含句。此字段提供叙实性推理所需的鉴别式，模型需要以背景句的内容来判断此句的真值情况。

（5）	`factivity`：叙实性判定。将模型对结论句真值情况的判断结果写入此字段。字段值包括`"TRUE"`, `"FALSE"`和`"UNCERTAIN"`三种选项。

（6）	`confidence`：对叙实性判定的置信度，即在`text`的条件下认为`hypothesis`在多大程度上为真或为假。当`factivity`字段值为`"TRUE"`或`"FALSE"`时，`confidence`字段取值范围为(0.50, 1.00]（左开右闭）；当`factivity`字段值为`"UNCERTAIN"`时，`confidence`字段值固定取`0.50`。

样例集数据包含`id`、`text`、`hypothesis`、`factivity`和`confidence`字段；测试集数据只包含`track`、`id`、`text`和`hypothesis`字段。
...
- 真值情况包括三种：
  - 如果模型认为，根据背景句的内容，结论句为真，则在`factivity`字段中写"TRUE"；同时在`confidence`字段中写入模型对该判定的置信度（即模型认为结论句在多大程度上为真），置信度的取值范围为(0.5, 1]，字段值类型为**数字**（数值保留到小数点后两位）；
  - 如果模型认为，根据背景句的内容，结论句为假，则在`factivity`字段中写入"FALSE"；同时在`confidence`字段中写入模型对该判定的置信度（即模型认为结论句在多大程度上为假），置信度的取值范围为(0.5, 1]，字段值类型为**数字**（数值保留到小数点后两位）；
  - 如果模型认为，根据背景句的内容，不能判断结论句的真假，则在`factivity`字段中写入"UNCERTAIN"；同时在`confidence`字段中写入`0.50`，字段值类型为**数字**（数值保留到小数点后两位）。
- 若模型拒绝回答，请调整提示词，重新测试。
- 若遇到其他问题，请邮件联系任务负责人。
...
# 7 评价标准（![Static Badge](https://img.shields.io/badge/20260501-UPDATE-blue?style=plastic)即将更新）

## 7.1 叙实性判定结果的分类

叙实性推理能力主要体现在认知主体对相关事件真值情况的判断上。本次评测采用 **[真值类别 + 置信度]** 的"双参数"模式来表示事件的真值情况。

- **真值类别**（`factivity`）：定类变量，表示认知主体对事件真假的基本判断，取值为 `"TRUE"`、`"FALSE"` 或 `"UNCERTAIN"`，分别表示：根据 `text` 字段的内容，可推知 `hypothesis` 字段的内容**为真**、**为假**，或**真假不能确定**。

- **置信度**（`confidence`）：定距变量，表示认知主体对上述判断的自信程度。当 `factivity` 为 `"UNCERTAIN"` 时，`confidence` 固定取 `0.50`；当 `factivity` 为 `"TRUE"` 或 `"FALSE"` 时，`confidence` 取值范围为 `(0.5, 1]`（左开右闭）。

根据上述双参数的取值组合，评测系统将每条数据的回答映射到以下九个**叙实性强度区间**之一：
...
为了更准确地评估大模型的叙实性推理能力，本次评测采用**梯度匹配**的方式计分，即：模型回答与标准回答所落入的得分区间越接近，得分越高。具体规则如下：

- 若模型回答与标准回答落入**同一区间**，得 **1 分**；
- 若模型回答所在区间与标准回答所在区间**相邻**，得 **2σ 分**（2σ ≈ 0.9545）；
- 若模型回答所在区间与标准回答**相隔一个区间**，得 **σ 分**（σ ≈ 0.6827）；
- 其他情况得 **0 分**。
...
> 注：σ ≈ 0.6827，对应正态分布一倍标准差范围内的概率值; 2σ ≈ 0.9545，对应正态分布两倍标准差范围内的概率值。

参赛队伍的**最终得分**为其在所有评测数据上所得分数的**总和**。
...
- Q3：样例集中出现的数据是否会在评测集中重复出现？
  ...
  A3：不会。按数据编号统计，样例集与评测集的数据不会完全重复。不过，在样例集中出现的谓词可能会在测评集中重复出现，但不会出现完全相同的两个句子。为了测试模型的泛化能力，样例集中出现的叙实性谓词数量将少于测试集。
...
- Q5: 今年为什么新增了shot数量的限制？
  ...
  A5: 今年在提示词设计中我们新增了一条对提示词中样例数量的要求：用于获取大模型对单一数据的回答而向其提供的样例（shot）数量不能超过3条（3-shot at most）。此要求是同时面向两个赛道的。这是因为，去年我们在统计评测结果时发现，支持长上下文的模型在提示词中存在大量回答样例的条件下，获得了远超其他测试方法的正确率提升。但这种做法事实上同样与人类的真实会话场景差异过大，因此不能真正测试出大模型能够在多大程度上像人类一样思考问题。

- Q6：模型回答是否需要参考世界知识？

  A6：模型在知识来源上的不同取向确实可能对叙实性判定结果产生一定影响，但此类偏差可通过提示词工程或模型微调等方式加以消除。建议参赛队伍在正式评测期间尝试多样化的评测手段，以获得更优结果。

- Q7：当`factivity`的字段值为`“UNCERTAIN”`时，为什么`confidence`字段值要统一填写0.50？

  A7：这主要是出于统一格式的考虑。当`factivity`的字段值为`“UNCERTAIN”`时，意味着听话人/大模型对被蕴含句的真值判定没有把握。从数据类型上来说，我们认为这种“不确定性”是一个与“对真值判定比较确定/有把握”相对的定类数据（nominal data），但没有程度上的区分。对于“真”或“假”，我们可以将其表述为“有60%的把握认为补足语小句所含事件为真”或“有90%的把握认为补足语小句所含事件为假”；但对于“不能确定”，“有20%的把握”与“有45%的把握”之间并无本质区别可言。因此，我们将UNCERTAIN的置信度固定为0.50，一方面是为了与其他两类标签保持格式一致，另一方面也是为了提醒参赛队伍，对于`factivity`的字段值为`TRUE`或`FALSE`的题目，其判定置信度的取值必须在0.50以上（不含0.50）。

- ![Static Badge](https://img.shields.io/badge/20260429-UPDATE-blue?style=plastic) Q8: 我们对4月份给定语料的标注结果与组织方意见不同，怎么办？

  A8: 我们也注意到部分样例集数据标注存在争议，我们正在商讨相应的解决方案。目前我们的调整方向包括两点：第一，调整标注者人员结构和标注说明，使标注结果具备更强的可解释性。第二，略微放宽评分的标准。目前的评分标准对模型打分的准确性要求较高，我们正在考虑扩大给分的答案范围，并提高相近区间的得分，从而使模型分值落在标准答案的相邻区间时不会被扣掉过多的分数。

- Q9: 是否会开会说明提交任务的事宜？是否有邮件通知？

  A9: 我们有计划在正式评测前召开一次任务说明会。不过由于目前仍有队伍陆续报名参赛，为了避免重复说明，我们将根据报名情况适时向参赛队发送邮件通知会议时间（预计为五月中旬）。

  Q10: 是否允许使用外部数据，即能否使用官方提供的数据以外的数据进行训练。比如是否可以自制数据，是否可以使用其他开源数据。是否允许使用伪标签，即测试集的数据能否用我们的模型推理一遍，作为扩充数据加入训练。

- ![Static Badge](https://img.shields.io/badge/20260531-UPDATE-blue?style=plastic) A10: 本次评测中可以调API，但禁止引入外部数据源。本次评测允许基于训练集通过模型预测产生伪标签，并加入训练过程。严禁以任何形式使用测试集（评测数据）的信息来生成伪标签或优化模型。即使您声称仅“打伪标签”，只要利用了测试集的样本或预测结果，即属于违规。总之，不能使用测试集来优化。

- Q11: 参数量是否有限制，比如大部分比赛限制10B以下的模型。以及如果使用多个模型，总参数量是否有限制？

  A11: 在提示词赛道中，模型的型号和数量均不做限制，队伍可自行决定选择何种闭源或开源模型；在微调赛道中，只要保证所用的每一个模型的参数都不超过10B即可，不限制所用模型的参数总量。
