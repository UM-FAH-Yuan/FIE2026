...
本次评测以JSON格式提供样例集和评测集。[样例集](sample%20sets)数据量约在500-1000条，评测集数量约在2000-4000条。语料由组织方团队筛选自相关真实语料库，并进行改编、标注与校对。
...
- 真值情况包括三种：
  ...
  - 如果模型认为，根据背景句的内容，不能判断结论句的真假，则在`factivity`字段中写入"UNCERTAIN"；同时在`confidence`字段中写入`0.5`，字段值类型为**数字**。
- 若模型拒绝回答，请调整提示词，重新测试。
- 若遇到其他问题，请邮件联系任务负责人。
...
- Q8: 我们对4月份给定语料的标注结果与组织方意见不同，怎么办？![Static Badge](https://img.shields.io/badge/20260429-UPDATE-blue?style=plastic) 
...
- Q12: 为什么第二批样例集的数据格式和第一批不同？![Static Badge](https://img.shields.io/badge/20260501-UPDATE-blue?style=plastic) 
